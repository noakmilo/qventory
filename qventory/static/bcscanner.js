(function(){
  ...
  function validateDetection(value) {
    ...
    // Si el mismo código aparece al menos 2 veces en las últimas 5 detecciones
  ...
  function startBDLoop(){
    ...
    const tick = async () => {
      try {
        frameCount++;
        // Solo procesar cada 3er frame para mejor rendimiento
        if (frameCount % 3 === 0) {
          const codes = await detector.detect(video);
          if (codes && codes.length){
            // Tomar el código con mayor confianza
    ...
    return new Promise(async (resolve, reject) => {
      for (const url of CANDIDATES) {
        try {
          ...
          console.warn('[Scanner] Failed', url, e); 
        }
      }
  ...
  async function flipCamera(){
    ...
    console.log(`[Scanner] Switching to camera ${currentDeviceIdx}: ${videoDevices[currentDeviceIdx].label}`);
  ...
  // Pre-cargar devices cuando se carga la página para mayor velocidad
  if ('mediaDevices' in navigator && 'enumerateDevices' in navigator.mediaDevices) {
    // Solo precargar si no es Safari iOS (para evitar prompts innecesarios)
    const isIOSSafari = /iPad|iPhone|iPod/.test(navigator.userAgent) || 
                       (navigator.userAgent.includes('Safari') && !navigator.userAgent.includes('Chrome'));
    if (!isIOSSafari) {
      navigator.mediaDevices.enumerateDevices()
        .then(devices => {
          const videoDevicesCount = devices.filter(d => d.kind === 'videoinput').length;
          console.log(`[Scanner] Found ${videoDevicesCount} video devices`);
        })
        .catch(() => {}); // Silently fail
    }
  }
