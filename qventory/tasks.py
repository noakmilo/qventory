...
@celery.task(bind=True, name='qventory.tasks.poll_ebay_new_listings_for_user', max_retries=120)
def poll_ebay_new_listings_for_user(self, user_id, job_id=None):
    """
    Manual user-scoped eBay polling refresh.

    The task waits behind any active import/polling work for the same user and
    then reuses poll_user_listings, which imports new listings and queues image
    hydration through Cloudinary when needed.
    """
    app = create_app()
    ...
    with app.app_context():
        ...
        if not job:
            ...
            db.session.commit()
            job_id = job.id
        elif job.celery_task_id != self.request.id:
        ...
        active_poll = PollingLog.query.filter(
            ...
            PollingLog.status == 'running',
            PollingLog.started_at >= stale_before,
        ...
        if active_job or active_poll:
            ...
            if self.request.retries >= self.max_retries:
                job.status = 'failed'
                job.completed_at = datetime.utcnow()
                job.error_message = 'Refresh timed out while waiting for another task to finish.'
                db.session.commit()
        ...
        if not credential:
            job.status = 'failed'
            job.completed_at = datetime.utcnow()
            job.error_message = 'eBay account not connected.'
            db.session.commit()
            return {'success': False, 'error': job.error_message}
        ...
        try:
            ...
            result = poll_user_listings(credential)
            errors = result.get('errors') or []
            new_listings = int(result.get('new_listings') or 0)

            job.status = 'failed' if errors else 'completed'
            job.completed_at = datetime.utcnow()
            job.total_items = new_listings
            job.processed_items = new_listings
            job.imported_count = new_listings
            job.error_count = len(errors)
            job.error_message = '; '.join(str(err) for err in errors)[:2000] if errors else None
            db.session.commit()
            ...
            if errors:
                Notification.create_notification(
                    ...
                    link_url='/inventory',
                    link_text='Back to Inventory',
                    source='manual_refresh'
                )
                return {'success': False, 'new_listings': new_listings, 'errors': errors}

            Notification.create_notification(
                user_id=user_id,
                type='success' if new_listings else 'info',
                title='Inventory refresh completed',
                message=(
                    f'{new_listings} new eBay listing(s) were imported.'
                    if new_listings else
                    'Active Inventory is already up to date.'
                ),
                link_url='/inventory',
                link_text='View Inventory',
                source='manual_refresh'
            )
            return {'success': True, 'new_listings': new_listings, 'errors': []}
...
def reconcile_user_finances(self, user_id, days_back=730):
    ...
    with app.app_context():
        ...
        try:
            sync_ebay_finances_user.run(user_id, days_back=days_back)
